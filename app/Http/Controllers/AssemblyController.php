...
class AssemblyController extends Controller
{
    ...
    public function get_raw_materials_item($item_classification, Request $request)
    {
        if ($request->autocomplete) {
            return DB::connection('mysql')->table('tabItem')
    ...
    public function duration_for_completed_assembly($prod)
    {
        ...
        return $this->format_operating_hrs($totalDuration);
    }
    ...
    {
        return DB::connection('mysql_mes')->table('job_ticket as tsd')
            ->join('process as p', 'p.process_id', 'tsd.process_id')
    ...
    public function get_reject_production_sched_monitoring($prodno)
    {
        ...
        return collect($orders)->sum('reject');
    }
    ...
    {
        if (Gate::denies('assign-production-order-to-machines')) {
            return redirect('/');
        }
    ...
    {
        if (Gate::allows('assign-production-order-to-machines')) {
            $values = [];
            if ($request->list) {

...
class SecondaryController extends Controller
{
    ...
    public function productionKanban(){
        $unscheduled_prod = DB::connection('mysql_mes')->table('production_order')
            ->whereNotIn('status', ['Completed', 'Stopped', 'Cancelled'])->where('is_scheduled', 0)
    ...
    public function getScheduledProdOrders_proScheduling($schedule_date){
        $orders = DB::connection('mysql_mes')->table('production_order')
            ->whereNotIn('status', ['Completed', 'Cancelled'])->where('is_scheduled', 1)
    ...
    public function get_bom_tracking($guide_id, $itemcode){
        ...
        return view('tracking_flowchart', compact('boms', 'item_codes','guide_id', 'production','bom', 'materials'));
    }
    ...
    public function reschedule_prod_details($prod){
        ...
        foreach($delivery_id as $row){
            ...
            $data[]=[
                'delivery_date'=> (empty($previous_row))? Carbon::parse($prod_details->rescheduled_delivery_date)->format('M-d-Y'): Carbon::parse($previous_row->previous_delivery_date)->format('M-d-Y'),                'delivery_reason' => $row->reschedule_reason,
                'remarks' => $row->remarks
            ];

...
class SecondaryController extends Controller
{
    ...
    public function operators_workstation_TaskList($workstation, $W_name)
    {
        try {
            $tasks = DB::connection('mysql')->table('tabTimesheet Detail AS tsd')
                ...
                ->where('wt.workstation', $workstation)->where('tsd.operation', 'Fabrication')
                ->where('ts.docstatus', 0)->where('tsd.status', $W_name)
                ->whereDate('ts.creation', '>', '2019-10-11')
    ...
    public function operatorpage($id)
    {
        $tabWorkstation = DB::connection('mysql')->table('tabWorkstation')
            ->where('name', $id)->select('production_line')->first();
    ...
    public function current_data_operator($workstation)
    {
        ...
        $workstations = DB::connection('mysql')->table('tabTimesheet Detail as tsd')
            ->join('tabTimesheet AS ts', 'ts.name', 'tsd.parent')
            ->join('tabWorkstation AS wt', 'wt.workstation_name', 'tsd.workstation')
            ->where('tsd.operation', 'Fabrication')->where('wt.production_line', $workstation)
            ->whereDate('tsd.creation', '>', '2019-10-11')->where('ts.docstatus', 0)
            ->select('tsd.*')->get();
    ...
    public function available_machine($machine)
    {
        $machine_available = db::connection('mysql')->table('tabMachine AS tm')
            ...
            ->where('tw.production_line', $machine)->select('tm.*', 'twm.parent')
            ->get();
    ...
    public function machine_breakdown(Request $request, $machine_code)
    {
        $machine = DB::connection('mysql_mes')->table('machine_breakdown as tmb')
            ->where('tmb.machine_id', $machine_code)->whereYear('date_reported', $request->year)
            ->whereMonth('date_reported', $request->month)->where('type', 'Breakdown')
            ->get();

        $machine1 = DB::connection('mysql_mes')->table('machine_breakdown as tmb')
            ->select('tmb.category')->whereYear('date_reported', $request->year)
            ->whereMonth('date_reported', $request->month)->where('type', 'Breakdown')
            ->where('tmb.machine_id', $machine_code)->groupBy('tmb.category')
            ->get();
    ...
    public function machine_corrective(Request $request, $machine_code)
    {
        $machine = DB::connection('mysql_mes')->table('machine_breakdown as tmb')
            ->where('tmb.machine_id', $machine_code)->whereYear('date_reported', $request->year)
            ->whereMonth('date_reported', $request->month)->where('type', 'Corrective')
            ->get();
        $machine1 = DB::connection('mysql_mes')->table('machine_breakdown as tmb')
            ->select('tmb.category')->whereYear('date_reported', $request->year)
            ->whereMonth('date_reported', $request->month)->where('type', 'Corrective')
    ...
    public function tbl_productionSchedule_report(Request $request)
    {
        try {
            $production_orders = DB::connection('mysql')->table('tabWork Order as tpo')
                ->where('tpo.docstatus', 1)->where('tpo.planned_start_date', $request->date)
                ->get();
    ...
    {
        $line = DB::connection('mysql_mes')->table('job_ticket as p')->where('p.workstation', $workstation)
            ->where('p.process_id', "!=", null)->groupBy('p.process_id')->select('p.process_id')->get();
    ...
    {
       return DB::connection('mysql')->table('tabWorkstation Machine')->where('parent', $workstation_name)->orderBy('machine_code', 'asc')->get();
    }

    public function machinename($schedule)
    {
        return DB::connection('mysql')->table('tabMachine')->where("name", $schedule)->select('machine_name')->first();
    }
    ...
    public function save_workstation(Request $request)
    {
        ...
        $now = Carbon::now();
        if (
            DB::connection('mysql')->table('tabWorkstation')
        ...
        if (
            DB::connection('mysql_mes')->table('workstation')
    ...
    public function getNextIdxProcessWorkstation($id)
    {
        $lastOrder = DB::connection('mysql_mes')->table('workstation_process')
            ->where('workstation_id', $id)->orderBy('created_at', 'desc')
            ->select('idx')->first();
    ...
    public function getNextIdxProcessWorkstation_erp($id)
    {
        $lastOrder = DB::connection('mysql')->table('tabWorkstation Process')
            ->where('parent', $id)->orderBy('creation', 'desc')
            ->select('idx')->first();
    ...
    public function process_profile($id)
    {
        ...
        $process = DB::connection('mysql_mes')->table('process')->where('id', $id)->first();

        $workstation = DB::connection('mysql_mes')->table('workstation')
            ->where('operation', 'Fabrication')->get();
    ...
    public function get_machine_assignment_jquery($workstation)
    {
        ...
        if ($workstation != null) {
            $process = DB::connection('mysql_mes')->table('workstation_machine')
                ...
                ->select('workstation_machine.*')->get();
    ...
    public function process_assignment(Request $request)
    {
        ...
        $now = Carbon::now();
        if (
            DB::connection('mysql_mes')->table('process_assignment')
    ...
    public function tbl_machine_setup_list(Request $request)
    {
        ...
        $workstation_list = DB::connection('mysql_mes')->table('workstation')->orderBy('order_no', 'asc')->get();
    ...
    public function countUnassignedTasksForOperator($workstation, $scheduleDate)
    {
        ...
        if ($scheduleDate <= $current_date) {
            $orders = DB::connection('mysql_mes')->table('production_order as prod')
                ->join('job_ticket as tsd', 'tsd.production_order', '=', 'prod.production_order')
                ->where('tsd.workstation', $workstation)->where('prod.status', 'Not Started')
                ->whereNull('tsd.machine')->whereNotNull('tsd.process')
                ->whereDate('prod.planned_start_date', "<=", $scheduleDate)->get();
        } else {
            $orders = DB::connection('mysql_mes')->table('production_order as prod')
                ->join('job_ticket as tsd', 'tsd.production_order', '=', 'prod.production_order')
                ->where('tsd.workstation', $workstation)->where('prod.status', 'Not Started')
    ...
    public function machineKanban_view_machineList($process_id, $workstation_id, $schedule_date)
    {
        ...
        $current_date = $now->toDateString();
        $scheduleDate = $schedule_date;
        $machine = DB::connection('mysql_mes')->table('process_assignment AS pm')
            ->join('machine AS m', 'm.machine_id', 'pm.machine_id')
            ->where('pm.process_id', $process_id)->where('pm.workstation_id', $workstation_id)
            ->select('pm.*', 'm.machine_name', 'm.machine_code')
            ->orderBy('m.machine_code', 'asc')->get();

        $workstation_name = DB::connection('mysql_mes')->table('workstation')->where('workstation_id', $workstation_id)->select('workstation_name')->first();
        $machine_loads = [];
        foreach ($machine as $row) {
            if ($schedule_date <= $current_date) {
                $completed_query = DB::connection('mysql_mes')->table('job_ticket as jt')
                    ...
                    ->join('production_order as prod', 'prod.production_order', 'jt.production_order')
                    ->join('process as p', 'p.process_id', 'jt.process_id')
                    ->where('tl.machine_code', $row->machine_code)->whereDate('jt.planned_start_date', $schedule_date)
                    ->whereNotIn('prod.status', ['Cancelled', 'Closed'])->where('jt.status', 'Completed')
                    ->where('prod.is_scheduled', 1)->where('jt.workstation', $workstation_name->workstation_name)
                ...
                $late_completed_query = DB::connection('mysql_mes')->table('job_ticket as jt')
                    ...
                    ->join('production_order as prod', 'prod.production_order', 'jt.production_order')
                    ->join('process as p', 'p.process_id', 'jt.process_id')
                    ->where('tl.machine_code', $row->machine_code)->whereDate('tl.to_time', $current_date)
                    ->whereNotIn('prod.status', ['Cancelled', 'Closed'])->where('jt.status', 'Completed')
                    ->where('prod.is_scheduled', 1)->where('jt.workstation', $workstation_name->workstation_name)
                    ->where('jt.process_id', $process_id)
                    ->select('prod.*', 'jt.workstation as workstation_plot', 'tl.machine_code as machine', 'tl.job_ticket_id as jtname', 'p.process_name', "jt.status as stat", 'tl.good as good_qty', 'tl.operator_name', 'tl.from_time', 'tl.to_time', 'tl.duration', 'tl.time_log_id', 'tl.machine_name', 'jt.remarks');
                ...
                $orders = DB::connection('mysql_mes')->table('time_logs as tl')
                    ...
                    ->join('process as p', 'p.process_id', 'jt.process_id')
                    ->where('tl.machine_code', $row->machine_code)->whereDate('jt.planned_start_date', '<=', $schedule_date)
                    ->whereNotIn('prod.status', ['Cancelled', 'Closed'])->where('tl.status', 'In Progress')
                    ->where('prod.is_scheduled', 1)->where('jt.workstation', $workstation_name->workstation_name)
                    ->where('jt.process_id', $process_id)->union($completed_query)->union($late_completed_query)
                    ->select('prod.*', 'jt.workstation as workstation_plot', 'tl.machine_code as machine', 'tl.job_ticket_id as jtname', 'p.process_name', "jt.status as stat", 'tl.good as good_qty', 'tl.operator_name', 'tl.from_time', 'tl.to_time', 'tl.duration', 'tl.time_log_id', 'tl.machine_name', 'jt.remarks')
                    ->get();
            } else {
                $orders = DB::connection('mysql_mes')->table('time_logs as tl')
    ...
    {
        if (Gate::denies('override-operator-timelog')) {
            return response()->json(['success' => 0, 'message' => 'Unauthorized.']);
        }
            ...
            if ($request->id) {
                ...
                if ($job_ticket_details->workstation == 'Painting') {
                    ...
                    DB::connection('mysql_mes')->table('time_logs')->insert([
                        ...
                        'good' => $pending,
                        'reject' => 0,
                        'machine_code' => $logs->machine_code,
                        'machine_name' => $logs->machine_name,
    ...
    {
        return DB::connection('mysql_mes')->table('process_assignment')->join('process', 'process.process_id', 'process_assignment.process_id')
            ->where('process_assignment.workstation_id', $workstation)->select('process.process_id', 'process.process_name')->distinct()->get();
    }
    ...
    {
        if (Gate::denies('print-job-ticket')) {
            return redirect()->back();
        }
    ...
    public function single_printJobTickets($production_order)
    {
        ...
        $prod_orders = DB::connection('mysql_mes')->table('production_order AS pro')
            ->where('pro.production_order', $production_order)->get();
    ...
    public function subquery_printTimesheet($prod_name)
    {
        $workstation_jbticket = DB::connection('mysql_mes')->table('job_ticket as jt')
            ->where('production_order', $prod_name)->groupBy('workstation')
    ...
    public function subquery_printTimesheets($prod_name, $workstation)
    {
        ...
        foreach ($workstations as $row) {
            if (
                ...
                if (
                    DB::connection('mysql_mes')->table('scrap_used')
                        ->where('production_order', '=', $prod_name)
                        ->exists()
                ) {
                    $remark = "with Scrap";
    ...
    public function add_shift(Request $request)
    {
        ...
        $checker = DB::connection('mysql_mes')->table('shift')->where('time_in', $request->time_in)
            ->where('time_out', $request->time_out)->where('shift_type', $request->shift_type)
            ->where('operation_id', $request->operation)->exists();
    ...
    public function add_operation(Request $request)
    {
        $now = Carbon::now();
        if (
            DB::connection('mysql_mes')->table('operation')
            ...
            if ($request->planned_start_datepicker == null) {
                ...
                if ($request->operation_id == 2) {
                    ...
                    foreach ($request->prodname as $i => $row) {
                        if (
                            DB::connection('mysql_mes')->table('job_ticket as jt')
                                ->join('time_logs as tl', 'jt.job_ticket_id',
                                    'tl.job_ticket_id')
                    ...
                    foreach ($request->prodname as $i => $row) {
                        if (
                            DB::connection('mysql_mes')->table('job_ticket as jt')
                                ->join('spotwelding_qty as spotpart', 'spotpart.job_ticket_id',
                                    'jt.job_ticket_id')
                                ->where('jt.production_order', $row)
    ...
    public function get_tbl_shiftsched_list(Request $request)
    {
        ...
        $shift_list = DB::connection('mysql_mes')->table('shift')
            ...
            ->where('shift.operation_id', $operation)->get();
    ...
    public function get_bom($bom, $guide_id, $item_code, $parent_item_code)
    {
        try {
            $bom1 = DB::connection('mysql')->table('tabBOM Item as bom')
                ...
                ->where('bom.docstatus', '<', 2)->where('bom.parent', $bom)
                ->select('bom.*', 'item.parts_category')->orderBy('bom.idx', 'asc')->get();

            $materials = [];
            foreach ($bom1 as $item) {
                $default_bom = DB::connection('mysql')->table('tabBOM')
                    ->where('docstatus', '<', 2)->where('is_default', 1)
                    ->where('item', $item->item_code)->first();
    ...
    public function production_mes($item_code, $sales_order)
    {
        ...
        if ($production != null) {
            ...
            $jt_details1 = DB::connection('mysql_mes')->table('job_ticket')
                ->where('production_order', $production->production_order)
                ->select(DB::raw('MAX(to_time) as to_time'))->first();
        ...
        if ($operation_id == 2) {
            ...
            foreach ($prod as $rows) {
                ...
                $data[] = array(
                    ...
                    'delivery_date' => ($rows->rescheduled_delivery_date == null) ? $rows->delivery_date : $rows->rescheduled_delivery_date,
                    //show reschedule delivery date or the existing delivery date based on validation
    ...
    public function get_production_painting()
    {
        ...
        foreach ($jobtickets_production as $row) {
            ...
            $production_order_status = DB::connection('mysql_mes')->table('production_order as pro')
                ...
                ->where('jt.workstation', 'Painting')->where('pro.production_order', $row->production_order)
                ->select('pro.status', 'jt.sequence')->first();
            ...
            $timelogs_inprogress = DB::connection('mysql_mes')->table('job_ticket as jt')
                ...
                ->join('process as p', 'p.process_id', 'jt.process_id')
                ->where('jt.production_order', $row->production_order)
                ->where('tl.status', "In Progress")->where('jt.workstation', 'Painting')
    ...
    public function getScheduledProdOrders1($schedule_date)
    {
        $orders = DB::connection('mysql_mes')->table('production_order as pro')
            ...
            ->where('jt.workstation', 'Painting')->whereDate('jt.planned_start_date', $schedule_date)
    ...
    public function selected_printJobTickets($prods)
    {
        ...
        $prod_orders = DB::connection('mysql_mes')->table('production_order')
            ->whereIn('production_order', $myArray)->get();
    ...
    public function operator_item_produced_report()
    {
        $workstation = DB::connection('mysql_mes')->table('workstation AS w')
            ->select("workstation_name", "workstation_id")
            ->where('operation', 'Fabrication')->orderBy('workstation_name', 'asc')
            ->get();
    ...
    public function getchangeover($date_from, $date_to, $operator_id, $workstation, $process_id, $parts)
    {
        ...
        foreach ($period as $date) {
            ...
            $get_all_operator = DB::connection('mysql_mes')->table('time_logs as tl')
                ...
                ->join('production_order as po', 'po.production_order', 'jt.production_order')
                ->join('process', 'process.process_id', 'jt.process_id')
                ->whereDate('tl.from_time', '=', $dates)->where('jt.workstation', $workstation)
                ->where('jt.process_id', $process_id)->where('po.parts_category', $parts)
                ->where('tl.duration', '<=', '15')->where('tl.operator_id', $operator_id)
    ...
    public function getprocess_query($workstation)
    {
        ...
        if ($workstation != "All") {
            $workstation = DB::connection('mysql_mes')->table('workstation')
                ->where('workstation_name', $workstation)->select('workstation_id')
                ->first();
    ...
    public function export_view($date_from, $date_to, $workstation, $process, $parts, $item_code)
    {
        $get_all_operator = DB::connection('mysql_mes')->table('job_ticket as jt')
            ...
            ->join('production_order as po', 'po.production_order', 'jt.production_order')
            ->join('process', 'process.process_id', 'jt.process_id')
            ->where('tl.operator_id', '!=', null)->where('tl.operator_name', '!=', null)
            ->whereDate('tl.from_time', '>=', $date_from)->whereDate('tl.to_time', '<=', $date_to)
            ->select('tl.operator_name', 'tl.operator_id', 'jt.workstation', 'jt.process_id', 'po.parts_category', 'process.process_name', 'po.item_code')
            ->groupBy('tl.operator_name', 'tl.operator_id', 'jt.workstation', 'jt.process_id', 'po.parts_category', 'process.process_name', 'po.item_code')
            ->get();
        ...
        foreach ($query as $rss) {
            $timelogs = DB::connection('mysql_mes')->table('job_ticket as jt')
                ...
                ->join('production_order as po', 'po.production_order', 'jt.production_order')
                ->whereDate('tl.from_time', '>=', $date_from)->whereDate('tl.to_time', '<=', $date_to)
                ->where('jt.workstation', $rss->workstation)->where('jt.process_id', $rss->process_id)
                ->where('po.parts_category', $rss->parts_category)->where('po.item_code', $rss->item_code)
                ->where('tl.duration', '<=', '15')->where('tl.operator_id', $rss->operator_id)
                ->select('tl.operator_name', 'jt.workstation', 'process.process_name', 'po.parts_category', 'tl.good', 'tl.duration', 'tl.reject')
                ->get();
    ...
    {
        $production_orders = DB::connection('mysql_mes')->table('production_order as po')
            ->select('po.item_code', 'po.description')->where('po.operation_id', '1')
            ->groupBy('po.item_code', 'po.description')->orderBy('po.item_code')
            ->get();
    ...
    public function get_reject_production_sched_monitoring($prodno)
    {
        $orders = DB::connection('mysql_mes')->table('job_ticket as tsd')
            ->leftJoin('time_logs', 'time_logs.job_ticket_id', 'tsd.job_ticket_id')
            ->join('process as p', 'p.process_id', 'tsd.process_id')
            ->where('tsd.production_order', $prodno)->where('tsd.workstation', 'Painting')
            ->join('workstation as work', 'work.workstation_name', 'tsd.workstation')
    ...
    public function count_current_painting_production_schedule_monitoring($schedule_date)
    {
        $orders_rejects = DB::connection('mysql_mes')->table('production_order as prod')
            ...
            ->join('process as p', 'p.process_id', 'tsd.process_id')
            ->where('tsd.workstation', 'Painting')->whereDate('tl.last_modified_at', $schedule_date)
            ->where('prod.is_scheduled', 1)->select("tsd.status", "tl.reject")->get();

        $orders_inprogress = DB::connection('mysql_mes')->table('production_order as prod')
            ->join('job_ticket as tsd', 'tsd.production_order', '=', 'prod.production_order')
            ->whereDate('tsd.planned_start_date', $schedule_date)->where('tsd.workstation', 'Painting')
            ->where('tsd.status', 'In Progress')->groupBy('prod.production_order', 'prod.qty_to_manufacture')
    ...
    public function update_reject_list(Request $request)
    {
        ...
        $rejectlist = [];
        if (
            DB::connection('mysql_mes')->table('reject_list')
    ...
    {
        if (
            DB::connection('mysql_mes')->table('qa_checklist')->where('reject_list_id', $request->delete_rejectlist_id)
                ->exists()
        ) {
    ...
    {
        if (Gate::denies('manage-item-classification-source')) {
            return response()->json(['success' => 0, 'message' => 'Unauthorized.']);
        }
    ...
    public function tbl_poweder_coat_consumed_list(Request $request)
    {
        ...
        foreach ($powder_data as $row) {
            $shift = DB::connection('mysql_mes')->table('shift')
                ...
                ->where('op.operation_name', 'Painting')->where('shift.shift_id', $row->operating_hrs)
                ->select('shift.*')->first();

            $item_details = DB::connection('mysql_mes')->table('fabrication_inventory as stock')
                ->where('stock.item_code', $row->item_code)->select('stock.uom')->first();
    ...
    public function get_powder_coat_item()
    {
        $output = "";
        $item_list = DB::connection('mysql')->table('tabItem as item')
            ->where('item.has_variants', "!=", 1)->whereIn('item.item_group', ['Raw Material'])
            ->where('item.item_classification', ['PA - Paints'])->where('item.item_name', 'like', '%powder%')
            ->orderBy('item.modified', 'desc')->select('item.name', 'item.item_name')->get();
    ...
    {
        $item_list = DB::connection('mysql')->table('tabItem as item')
            ->where('item.name', $item)->select('item.item_name', 'item.name')->first();
    ...
    {
        if (Gate::denies('override-production-order')) {
            return response()->json(['success' => 0, 'message' => 'Unauthorized.']);
        }
        ...
        if ($request->complete_operation_id == "0") {
            ...
            if (count($orders) == 0) {
                $prod_details = DB::connection('mysql_mes')->table('production_order')
                    ...
                    ->select('qty_to_manufacture')->first();
                $val = [
                    "status" => "Completed",
    ...
    {
        if (Gate::denies('manage-rescheduled-delivery-reason')) {
            return response()->json(['success' => 0, 'message' => 'Unauthorized.']);
        }
    ...
    public function update_late_delivery(Request $request)
    {
        ...
        if (
            DB::connection('mysql_mes')->table('delivery_reschedule_reason')
    ...
    {
        if (
            DB::connection('mysql_mes')->table('reject_material_type')
    ...
    {
        if (Gate::denies('manage-production-order-cancellation')) {
            return response()->json(['success' => 0, 'message' => 'Unauthorized.']);
        }
    ...
    public function update_reason_for_cancellation(Request $request)
    {
        ...
        if (
            DB::connection('mysql_mes')->table('reason_for_cancellation_po')

    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-body">
          <div class="row">
            <div class="col-md-12">
              <ul class="nav nav-tabs" role="tablist" style="font-size: 14pt;">
                <li class="nav-item">
                  <a class="nav-link active" id="current-production-order-tab" data-toggle="tab" href="#current-tab" role="tab">Current Job Ticket</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link" id="production-queue-tab" data-toggle="tab" href="#production-queue" role="tab">Scheduled Task(s)</a>
                </li>
              </ul>
...
@include('modals.enter_reject_modal')
@include('modals.logout_confirmation_modal')
@include('modals.quality_check')
@include('quality_inspection.modal_inspection')

@include('modals.end_scrap_task')

@include('modals.enter_scrap_modal')
  ...
  $(document).ready(function(){
    ...
    $(document).on('click', '.add-helper-btn', function(e){
      ...
      $(document).on('click', '.view-in-progress-operator', function(e){
        ...
        $.ajax({
          url:"/view_operator_task/"+job_ticket_id+"/"+operator_id,
          type:"GET",
          success:function(data){
            var row = '';
            $.each(data, function(i, d){
    ...
    function getAssignedTasks(){
      $.ajax({
        url:"/operator_scheduled_task/{{ $workstation }}/{{ $job_ticket_details->process_id }}",
        type:"GET",
        success:function(data){
    ...
    function get_current_job_ticket_tasks(){
      ...
      $.ajax({
        url:"/get_current_operator_task_details/{{ Auth::user()->user_id }}",
        type:"GET",
        data: data,
        success:function(data){
    ...
    $(document).on('click', '.end-task-btn', function(e){
      ...
      if (production_order.split('-')[0] == 'SC') {
        var description = $(this).data('description');
        description = description.split(', ');
        $('#end-scrap-task-modal .timelog-id').val($(this).data('timelog-id'));
        $('#end-scrap-task-modal .process-name').text($(this).data('process-name'));
        $('#end-scrap-task-modal .max-qty').text($(this).data('qty-mfg'));
        $('#end-scrap-task-modal .production-order').text(production_order);
        $('#end-scrap-task-modal .production-order-input').val(production_order);
        $('#end-scrap-task-modal .workstation-input').val('{{ $workstation }}');
        $('#end-scrap-task-modal .balance-qty').val($(this).data('qty-mfg'));
        $('#end-scrap-task-modal input[name="material"]').val(description[0]);
        $('#end-scrap-task-modal input[name="length"]').val(0);
        $('#end-scrap-task-modal input[name="width"]').val(0);
        $('#end-scrap-task-modal input[name="thickness"]').val(description[1].replace(/[^0-9.]/g,''));
        $('#material-density').val($(this).data('density'));
        $('#end-scrap-task-modal').modal('show');
    ...
    function change_total_scrap_remaining(){
      var balance = $('#end-scrap-task-modal input[name="balance_qty"]').val();
      
      var l = $('#end-scrap-task-length').val();
      var w = $('#end-scrap-task-width').val();
      var t = $('#end-scrap-task-thickness').val();
      var m = $('#end-scrap-task-material').val();
      var q = $('#end-scrap-task-qty').val();
      var d = $('#material-density').val();
      ...
      $('#total-scrap-remaining').text(balance - used_kg);
      $('#end-scrap-task-modal input[name="completed_qty_kg"]').val(used_kg);
    }

    $(document).on('focus', '#end-scrap-task-modal input[type=text]', function() {
      if($(this).data('edit') > 0){
        end_scrap_task_active_input = $(this).attr('id');
      }else{
        end_scrap_task_active_input = null;
      }
    });

    $(document).on('click', '#end-scrap-task-modal .num', function() {
      $("#" + end_scrap_task_active_input).focus();
      var input = $('#end-scrap-task-modal #' + end_scrap_task_active_input);
      var x = input.val();
      var y = $(this).text();
      ...
      change_total_scrap_remaining();
    });

    $(document).on('click', '#end-scrap-task-modal .del', function() {
      $("#" + end_scrap_task_active_input).focus();
      var input = $('#end-scrap-task-modal #' + end_scrap_task_active_input);
      var x = input.val();

      if($('#end-scrap-task-modal #' + end_scrap_task_active_input).data('edit') > 0){
        input.val(x.substring(0, x.length - 1));
    ...
    $('#end-scrap-task-modal form').submit(function(e){
      e.preventDefault();
      var balance_qty = $('#end-scrap-task-modal form input[name="balance_qty"]').val();
      var completed_qty = $('#total-scrap-remaining').text();
      
      if(parseFloat(completed_qty) <= 0){
        showNotification("danger", "Completed Qty cannot be less than or equal to <b>0</b>", "now-ui-icons travel_info");
        return false;
      }

      if(parseFloat(completed_qty) > parseFloat(balance_qty)){
        showNotification("danger", "Completed Qty cannot be greater than <b>" + parseFloat(balance_qty) + "</b>", "now-ui-icons travel_info");
        return false;
      }
      ...
      $.ajax({
        ...
        success:function(data){
          if (data.success < 1) {
            ...
            showNotification("success", data.message, "now-ui-icons ui-1_check");
            get_current_job_ticket_tasks();
            $('#end-scrap-task-modal').modal('hide');
   ...
   var goodScan = function (cardData) {
    ...
    var scrap_active_input = null;
	
    $(document).on('click', '.enter-scrap-btn', function(e){
      e.preventDefault();
      scrap_active_input = null;

      var production_order = $(this).data('production-order');
      var job_ticket_id = $(this).data('jobticket-id');
      $('#scrap-production-order').text(production_order);
      $('#scrap-job-ticket-id').val(job_ticket_id);

      var item_code = $(this).data('item-code');
      $.ajax({
        url: "/get_item_specs/" + item_code,
        type:"GET",
        success:function(response){
          $('#scrap-material').val(response.material);
          // $('#scrap-length').val(response.length);
          // $('#scrap-width').val(response.width);
          $('#scrap-thickness').val(response.thickness);
          $('#enter-scrap-modal').modal('show');
        },
        error: function(jqXHR, textStatus, errorThrown) {
          console.log(jqXHR);
          console.log(textStatus);
    ...
    $(document).on('focus', '#enter-scrap-frm input[type=text]', function() {
      if($(this).data('edit') > 0){
        scrap_active_input = $(this).attr('id');
      }else{
        scrap_active_input = null;
      }
    });

    $(document).on('click', '#enter-scrap-frm .num', function() {
      $("#" + scrap_active_input).focus();
      var input = $('#enter-scrap-frm #' + scrap_active_input);
      var x = input.val();
      var y = $(this).text();
    ...
    $(document).on('click', '#enter-scrap-frm .del', function() {
      $("#" + scrap_active_input).focus();
      var input = $('#enter-scrap-frm #' + scrap_active_input);
      var x = input.val();
    ...
    $('#enter-scrap-frm').submit(function(e){
      e.preventDefault();

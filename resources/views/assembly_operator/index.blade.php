    ...
    <div class="row">
      <div class="col-md-12 text-white">  
          <table style="width: 97%; margin-left: 15px;">  
            ...
            <td style="width: 55%;">
              <h2 class="title" style="margin: 2px auto; padding-left: 10px;">{{ $machine_details->machine_name }}</h2>
            </td>
          </tr>
        </table>
      </div>
     ...
     <div class="col-md-12">
      ...
      <table style="width: 100%; display: block; overflow-x: auto; white-space: nowrap;">
        <tr>
          ...
          <td>
            <a href="/assembly_operator/{{ $row->machine_code }}" class="custom-a">
              <div class="card p-2" style="width: 180px; height: 80px; margin: 5px; font-size: 14pt;">
                <div style="white-space: normal; margin: 10px auto;"><b>{{ $row->machine_name }}</b></div>
              </div>
            </a>
          </td>
          @endforeach
    ...
    <div class="modal-dialog modal-xl" role="document">
      <div class="modal-content">
        <div class="modal-header" style="background-color: #6A1B9A;">
          <h5 class="modal-title" id="exampleModalLabel" style="color: #fff">Maintenance Request [<b>Wiring & Assembly</b>]</h5>
...
<script>
  $(document).ready(function(){
    ...
    function get_pending_for_maintenance(){
      $.ajax({
        url:"/operator/pending_for_maintenance/3",
        type:"GET",
        success:function(data){
    ...
    function get_production_order_task(production_order, workstation){
      $.ajax({
        url: '/get_production_order_task/PROM-' + production_order + '/' + workstation + '?assembly_operator=1',
        type:"GET",
        success:function(response){
          if (response.success > 0) {
            ...
            $('#scan-production-order-step3-modal input[name="production_order"]').val(response.details.production_order.production_order);
            $('#scan-production-order-step3-modal input[name="machine_code"]').val(response.details.machine_code);
            ...
            $('#scan-production-order-step3-modal .modal-title').html('Scan Operator ID [<b>' + workstation + '</b>]');
            $('#scan-production-order-step3-modal').modal('show');
          } else {
      ...
      $.ajax({
        ...
        success:function(data){
          if (data.success < 1) {
            ...
            window.location.href="/assembly_dashboard/"+data.details.machine_code+"/"+data.details.production_order;
          }
        }, 
        error: function(jqXHR, textStatus, errorThrown) {
          console.log(jqXHR, textStatus, errorThrown);
          showNotification("danger", 'Something went wrong. Please try again.', "now-ui-icons travel_info");
        }
      });  
    }

...
<div class="panel-header">
   <div class="header text-center" style="margin-top: -65px;">
        		...
        		<table style="text-align: center; width: 87%;">
					<tr>
						...
						<td style="width: 25%;" rowspan="2">
							<h2 class="title text-left" style="margin: auto 10px; font-size: 18pt;">{{ $machine_details->machine_name }}</h2>
						</td>
						<td style="width: 37%;">
    ...
    <input type="hidden" id="current-production-order" value="{{ $production_order }}">
  	</div>
</div>
							...
							<ul class="nav nav-tabs" role="tablist" style="font-size: 10pt;">
								<li class="nav-item">
									<a class="nav-link active" id="current-production-order-tab" data-toggle="tab" href="#current-tab" role="tab">
										<b>Current Job Ticket ({{ $production_order }})</b>
									</a>
								</li>
								<li class="nav-item">
									<a class="nav-link" id="production-queue-tab" data-toggle="tab" href="#production-queue" role="tab"><b>Scheduled Task(s)</b></a>
								</li>
							</ul>
...
<script>
	$(document).ready(function(){
      ...
      $(document).on('click', '.view-in-progress-operator', function(e){
		...
		function getAssignedTasks(){
			$.ajax({
				// url:"/get_assigned_tasks/orkstation }}/machine_details->machine_code }}",
				type:"GET",
				success:function(data){
		...
		function get_current_job_ticket_tasks(){
			...
			$.ajax({
				url:"/get_assembly_current_operator_task_details/{{ Auth::user()->user_id }}",
				type:"GET",
				data: data,
				success:function(data){
    ...
    $(document).on('click', '.end-task-btn', function(e){
      ...
      $('#end-task-modal .workstation-input').val('1workstation }}');
    ...
    $(document).on('click', '.restart-task-btn', function(e){
      ...
      $('#restart-task-modal .workstation').html('[<b>{1 $workstation }}</b>]');
    ...
    $(document).on('click', '.start-task-btn', function(e){
      ...
      var count_selected_process = $('#select-part-div .selected-part').length;
      if (count_selected_process <= 0) {
        $(this).removeAttr('disabled');
        showNotification("danger", 'Please select process to start task.', "now-ui-icons travel_info");
        return false;
      }

      var job_ticket_ids = [];
      $('#select-part-div .selected-part').each(function(){
        job_ticket_ids.push($(this).data('jobticketid'));
      });
    ...
    $(document).on('click', '.enter-reject-btn', function(e){
      ...
      $('#enter-reject-modal .workstation-input').val('1$workstation }}');

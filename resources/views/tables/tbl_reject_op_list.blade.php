...
<table class="table table-striped">
        ...
        <tbody>
          @forelse($reject_list as $index => $row)
            ...
            <td>
              ...
              <button type='button' class='btn btn-default hover-icon btn-delete-rejectlist' data-id="{{ $row->reject_list_id }}" data-ctgID="{{ $row->reject_category_id }}" data-rjtlist="{{ $row->reject_checklist }}" data-rjtreason="{{ $row->reject_reason }}"  data-reloadtbl="Operator"><i class='now-ui-icons ui-1_simple-remove'></i></button>
      ...
      <center>
<div id="op_reject_check_list_pagination" class="col-md-12 text-center" style="text-align: center;">
{{ $reject_list->links() }}
</div>
</center>
</div>

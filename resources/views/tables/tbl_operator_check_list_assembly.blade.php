          ...
          @forelse($check_list as $index => $row)
            ...
            <td>
                <a href="#" class="hover-icon"  data-toggle="modal" style="padding-left: 5px;">
                  <button type='button' class='btn btn-default btn-delete-opchecklist' data-id="{{$row->operator_reject_list_setup_id}}" data-workstation="{{$row->workstation_name}}" data-rejectchecklist="{{$row->reject_reason}}" data-operation="{{$row->operation_name}}" data-reloadtbl="Assembly"><i class='now-ui-icons ui-1_simple-remove'></i></button>
                </a>
            </td>
            @empty
            <tr>

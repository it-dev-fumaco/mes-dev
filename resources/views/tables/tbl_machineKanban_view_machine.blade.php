            ...
            <tr>
              ...
              <td style="padding-right: 10px;">
                <div class="card justify-content-center" style="overflow-y: auto;width: 350px; padding:0px;">
                              <div class="card-header justify-content-center" style="background-color: #f96332; color: white;line-height: 18px; margin-top: -10px;padding-bottom: 1px;">
                                <h6 class="card-title text-center" style="font-size: 2vw;font-size: 13px;">
                                  <b>{{ $rowws['machine_code'] }} - {{ $rowws['machine_name'] }}</b>
                                </h6>
                              </div>
                              <div class="card-body connectedSortable sortable_list" style="height: 500px; position: relative;" id="{{ $rowws['machine_code'] }}" data-mprocess="{{ $rowws['machine_process']}}" data-status="Accepted">
                                @foreach($rowws['machine_load'] as $rowsss)
                                @php
                                ...
                                @endphp
                                <div id="sched_orders" class="card {{ $rowsss->stat}} {{ $blickme }}" data-index="{{ $rowsss->jtname }}" data-position="{{ $rowsss->order_no}}" data-card="{{ $rowsss->machine }}" data-qtyaccepted="0" data-mprocess="{{ $rowsss->process_name }}" data-prod="{{ $rowsss->production_order }}" data-sales="{{ ($rowsss->sales_order == '')? $rowsss->material_request : $rowsss->sales_order  }}" data-customer="{{ $rowsss->customer }}" data-desc="{{ $rowsss->description }}" data-proditem="{{ $rowsss->item_code }}" data-qty="{{ number_format($rowsss->qty_to_manufacture) }} {{ $rowsss->stock_uom }}" data-cpt="{{ ($rowsss->remarks == 'Override')? number_format($rowsss->good_qty):number_format($rowsss->qty_to_manufacture) }} {{ $rowsss->stock_uom }}" data-process=" -[{{ $rowsss->process_name }}]" data-mprocessvalidation="{{ $rowsss->process_name }}" data-delivery="{{ date('m-d-Y', strtotime($rowsss->delivery_date))}}" data-planstartdate="{{ date('m-d-Y', strtotime($rowsss->planned_start_date)) }}" data-modaltitle="{{ $rowsss->production_order }} -[{{ $rowsss->process_name }}]" data-operatorname="{{ $rowsss->operator_name }}" data-machinecode="[ {{ $rowsss->machine }} ]" data-starttime="{{ date('Y-m-d h:i:s a', strtotime($rowsss->from_time)) }}" data-totime="{{ date('Y-m-d h:i:s a', strtotime($rowsss->to_time)) }}" data-duration="{{ $convert_timee }}" data-machinename="{{ $rowsss->machine_name }}" data-taskstatus="{{ $rowsss->stat }}" data-cycletime="{{ number_format($rowsss->good_qty) }}" style="background-color: {{ $colorme }};margin-top: -10px;margin-bottom: 10px;height: 30px;width: 300px;"  data-ws="{{ $rowws[ 'workstation_name' ] }}" data-wsid="{{ $rowws['workstation_id'] }}" data-qty-accepted="{{ number_format($rowsss->qty_to_manufacture) }}" data-timelog="{{ $rowsss->time_log_id }}" data-remark="{{ $rowsss->remarks }}">
                                      ...
                                      <tr>
                                        <td>
                                          ...
                                          <div class="details-pane">
                                            <h5 class="title">{{ $rowsss->production_order }}<span>[{{ ($rowsss->stat == "Accepted") ? "Not Started": $rowsss->stat }}]</span></h5>
                                                      <p class="desc">

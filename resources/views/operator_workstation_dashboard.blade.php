...
<script>
  $(document).ready(function(){
    ...
    $('#view-schedule-btn').click(function(e){
      e.preventDefault();
      $.ajax({
        url:"/view_conveyor_schedule/" + workstation,
        type:"GET",
        success:function(data){
          $('#view-scheduled-task-tbl').html(data);
          $('#view-conveyor-schedule-modal').modal('show');
        }
      });
    });
    ...
    $(document).on('click', '#rc-numpad .num', function() {
      $("#rejected-qty").focus();
      var input = $('#rejected-qty');
      var x = input.val();
      var y = $(this).text();
    ...
    $(document).on('click', '#rc-numpad .clear', function() {
      $('#rejected-qty').focus();
      var input = $('#rejected-qty');
      input.val(0);
    });

    $(document).on('click', '#rc-numpad .del', function() {
      $("#rejected-qty").focus();
      var input = $('#rejected-qty');
      var x = input.val();
  ...
  $(document).on('click', '#reject-confirmation-frm .prev-tab', function() {
    ...
    $(document).on('click', '#scan-production-order-step3-modal .submit-enter-operator-id', function(e) {
      ...
      var scrap_id = $('#scan-production-order-step3-modal input[name="scrap_id"]').val();
      if (scrap_id > 0) {
        insert_scrap_job_ticket();
      ...
      $.ajax({
        ...
        success:function(data){
          if (data.success < 1) {
            ...
            window.location.href="/operator_dashboard/"+data.details.machine_code+"/"+workstation+"/"+data.details.job_ticket_id;
          }
        }, 
        error: function(jqXHR, textStatus, errorThrown) {
          console.log(jqXHR);

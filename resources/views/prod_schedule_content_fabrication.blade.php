<ul class="nav nav-tabs" id="workstation-tab-list" role="tablist">
  <li class="nav-item">
    <a class="nav-link active" id="production-output-tab" data-toggle="tab" href="#production-output" role="tab" aria-controls="fabrication" aria-selected="true">Production Output Summary</a>
  </li>
  @foreach($workstations as $i => $row)
  <li class="nav-item">
    <a class="nav-link" id="wtab{{$i}}" data-toggle="tab" href="#w{{$i}}" role="tab" aria-controls="fabrication" aria-selected="false">{{ $row->name }}</a>
  </li>
  @endforeach
</ul>
<!-- Tab panes -->
<div class="tab-content" style="min-height: 620px;">
  <div class="tab-pane active" id="production-output" role="tabpanel" aria-labelledby="production-output-tab">
    <br>
    <div class="row">
      @forelse($output as $row)
      @if(count($row['output']) > 0)
      {{-- @php
        $machines = collect($output['output'])->where('workstation', $row->name);
      @endphp --}}
      <div class="col-md-3">
  ...
  @endphp
  <div class="tab-pane" id="w{{$i}}" role="tabpanel" aria-labelledby="wtab{{$i}}">

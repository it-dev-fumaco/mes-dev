<table class="table custom-table-striped text-center">
   ...
   <tbody style="font-size: 9pt;">
         ...
         <tr style="display:{{(number_format($row['qty']) <= $row['feedback_qty'])? 'none':''}};">
            <td class="text-center" rowspan="2"><span class="badge badge-info" style="font-size: 9pt;margin-right:10px;"></span><a href="#" data-jtno="{{ $row['production_order'] }}" class="prod-details-btn" style="color: black;">{{ $row['production_order'] }}</a></td>
             ...
             <div class="btn-group">
               ...
               <div class="dropdown-menu">
                ...
                {{--<a class="dropdown-item editcpt_qty" href="#" id="editcpt-qty-btn" data-prod="{{$row['production_order']}}" data-qty="{{ $row['qty'] }}">Edit</a>--}}
                 <a class="dropdown-item view-process-qty-btn" href="#" id="view-process-qty-btn" data-prod="{{$row['production_order']}}" data-qty="{{ $row['qty'] }}">View Process</a>
               </div>
             </div>
           </td>
         </tr>
         ...
         <tr class="heightcustom" style="background-color: {{ $colorme }};color:white; display:{{(number_format($row['qty']) <= $row['feedback_qty'])? 'none':''}};">
           ...
           <td class="text-center" colspan="{{ $colpsn }}" style="height: 20px;border: none;display:none;">{{ ($row['prod_status'] == 'Completed')? $row['duration']:'' }} </td>
